    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath, buffer_id=buffer_id,
                                    idle_timeout=1, hard_timeout=30, priority=priority, match=match,
                                    instructions=inst, flags=ofproto.OFPFF_SEND_FLOW_REM)
        else:
            mod = parser.OFPFlowMod(datapath=datapath, priority=priority,
                                    idle_timeout=1, hard_timeout=30, match=match, instructions=inst, flags= ofproto.OFPFF_SEND_FLOW_REM)
