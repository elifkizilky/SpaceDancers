...
def print_data_table(data):
    ...
    table = PrettyTable()
    table.field_names = ['Switch', 'Last Packet In', 'Last Removed', 'Last Duration', '# of Packets', 'Last Timeout']
    ...
    for switch, switch_data in data.items():
        ...
        last_timeout = switch_data.get('last_timeout', 'N/A')
        table.add_row([switch, last_packet_in, last_removed, last_duration, packet_count, last_timeout])
...
class SimpleMonitor13(app_manager.RyuApp):
    ...
    def _monitor(self):
        while True:
            for dp in self.datapaths.values():
                ...
                # match = OFPMatch(
                #     in_port=4,  # Match on the input port
                #     eth_dst='00:00:00:00:00:01',  # Match on the destination MAC address
                #     eth_src='00:00:00:00:00:04'   # Match on the source MAC address
                # )
                # self.remove_flow_with_match(dp,match)
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        print("MATCH",match)
        existing_flow_attributes = self.data_table.get(match, {})
        print("Flow attributes", existing_flow_attributes)
        ...
        self.data_table[match] = existing_flow_attributes
    ...
    def remove_flow(self, datapath, cookie):
        ...
        datapath.send_msg(mod)
    def remove_flow_with_match(self, datapath, match):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        mod = parser.OFPFlowMod(
            datapath=datapath,
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        for stat in ev.msg.body:
            ...
            match = stat.match  # Assuming cookie is unique for each flow
            if match in self.data_table:
                # Assuming you have some existing values for last_packet_in and last_removed
                existing_last_packet_in =  self.data_table.get(match).get('last_packet_in')  # Replace with actual existing value
                existing_last_removed = self.data_table.get(match).get('last_removed')  # Replace with actual existing value
                ...
                self.data_table[match] = flow_attributes
    ...
    def flow_removed_handler(self, ev):
       ...
       msg.packet_count, msg.byte_count, msg.match)
        ...
        match= msg.match  # Assuming cookie is unique for each flow
