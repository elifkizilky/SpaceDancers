...
class SimpleMonitor13(app_manager.RyuApp):
    ...
    def _monitor(self):
        ...
        while True:
            ...
            for dp in datapaths_snapshot:
                ...
                print("TOTAL HİT COUNT", lookup_count_diff-total_packet_in_count - rejected_flows)
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath, buffer_id=buffer_id,
                                    idle_timeout=allocatedTimeout, hard_timeout=5, priority=priority, match=match,
                                    instructions=inst, flags= flags)
                
        else:
            mod = parser.OFPFlowMod(datapath=datapath, priority=priority,
                                    idle_timeout=allocatedTimeout, hard_timeout=5, match=match, instructions=inst, flags= flags)
